...
; c. query: (meeting-time (Hacker Alyssa P) (Wednesday ?x))
...
(define (uniquely-asserted operands frame-stream)
	(stream-flatmap
		(lambda (frame)
			(let ((qevaled (qeval (unique-query operands) (singleton-stream frame))))
				(if (eq? (stream-length qevaled) 1)
					qevaled
					the-empty-stream)))
		frame-stream))
...
;; shortcut for driver loop
(define (q) (query-driver-loop))
